...
st.write("""
Interpretations for the Random Forest Credit Data Predictions
""")

st.subheader('Results')
default_type=st.multiselect('Select a Default Type', data.DEFAULTS.unique(), default=None)
new_data=data[data.DEFAULTS.isin(default_type)]
st.write(new_data.iloc[:,1:])

st.subheader('Chart')
pred=st.selectbox('X', data.columns[0:3])
deft=st.selectbox('Y', data.columns[0:3])
