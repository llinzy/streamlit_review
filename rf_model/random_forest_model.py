...
print(f'\n\nTop five features of importance:\n{list(zip(r_imf,r_irt))}\n')
...
rf_exp_par_confusion1=confusion_matrix(test_labels, rf_prdt)

print("RF Confusion Matrix:\n\n {}".format(rf_exp_par_confusion1))

rf_exp_par_class_report1=classification_report(test_labels, rf_prdt)
print("\n\n RF Classification Report:\n\n {}".format(rf_exp_par_class_report1))

print(metric(rf_exp_par_confusion1))

print(f'\n\nThe metric of interest for this business solution is precision, \
as the focus is to predict DEFAULTS(1).  By establishing a threshold for prediction values to\
account for the disproportion in DEFAULTS(1) in the data, the model is able to improve the\
predictiblity of DEFAULTS(1).')


"""
Considering the disproportion within the distribution of classes, a
threshold is added to coerce balance into class distribution
"""

print(f"Class distribution:\n\n {test_labels.DEFAULT.value_counts()}\n")

trec=np.where(rf_predict>.15, 1,0)

print(f'Apply a threshold adjustment greater than 15%\n')
# In[16]:


#incorporate prediction results into the confusion matrix and generate metric reports
rf_exp_par_confusion=confusion_matrix(test_labels, trec)

print("RF Confusion Matrix:\n\n {}".format(rf_exp_par_confusion))
rf_exp_par_class_report=classification_report(test_labels, trec)
...
fpr_rf2, tpr_rf2, _ = precision_recall_curve(test_labels, trec)
...
"""
print("RF Average Precision/Recall Score {}".format(round(average_precision_score(test_labels, trec),4
)))
...
plt.plot(fpr_rf2, tpr_rf2, label="RF Average Precision/Recall Score {}".format(round(average_precision_score(test_labels, trec),4)))
...
fpr_rf, tpr_rf, _= roc_curve(test_labels, trec)
...
print("RF ROC Score {}".format(round(roc_auc_score(test_labels,trec), 4)))
